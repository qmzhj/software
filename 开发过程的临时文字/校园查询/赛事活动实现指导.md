...
当前 `query.html` 已有基础详情弹窗，需扩展为包含"赛事参与人"栏目的完整界面：

**修改 `loadEvents()` 中的点击事件**，改为调用 `showEventDetail(eid)` 从 API 加载数据。
...
在 `query.html` 的赛事详情弹窗中，增加小组列表渲染区。小组数据从详情 API 获取。每个小组卡片包含：
